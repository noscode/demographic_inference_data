...
def model_func(params, ns):
    ...
    """
    nuAf, nuB, nuEu0, nuEu, nuAs0, nuAs, nuKHV0, nuKHV, nuJp0, nuJp, mAfB, mAfEu, mAfAs, mEuAs, mChKHV, mChJp, TAf, TB, TEuAs, TChKHV, TChJp = params
    ...
    fs = moments.Manips.split_3D_to_4D_3(fs, ns[2] + ns[4], ns[3])

    nuEu_func = lambda t: nuEu0 * (nuEu / nuEu0) ** ((t + TEuAs) / (TEuAs + TChKHV + TChJp))
    nuAs_func = lambda t: nuAs0 * (nuAs / nuAs0) ** ((t + TEuAs) / (TEuAs + TChKHV + TChJp))
    nuKHV_func = lambda t: nuKHV0 * (nuKHV / nuKHV0) ** (t / (TChKHV + TChJp))
    nu_func = lambda t: [nuAf, nuEu_func(t), nuAs_func(t), nuKHV_func(t)]
    m = np.array([[0, mAfEu, mAfAs, 0],
                  [mAfEu, 0, mEuAs, 0],
                  [mAfAs, mEuAs, 0, mChKHV],
                  [0, 0, mChKHV, 0]])
    fs.integrate(nu_func, TChKHV, m=m)

    fs = moments.Manips.split_4D_to_5D_3(fs, ns[2], ns[4])

    nuEu_func = lambda t: nuEu0 * (nuEu / nuEu0) ** ((t + TEuAs + TChKHV) / (TEuAs + TChKHV + TChJp))
    nuAs_func = lambda t: nuAs0 * (nuAs / nuAs0) ** ((t + TEuAs + TChKHV) / (TEuAs + TChKHV + TChJp))
    nuKHV_func = lambda t: nuKHV0 * (nuKHV / nuKHV0) ** ((t + TChKHV) / (TChKHV + TChJp))
    nuJp_func = lambda t: nuJp0 * (nuJp / nuJp0) ** (t / (TChJp))
    nu_func = lambda t: [nuAf, nuEu_func(t), nuAs_func(t), nuKHV_func(t), nuJp_func(t)]
    m = np.array([[0, mAfEu, mAfAs, 0, 0],
                  [mAfEu, 0, mEuAs, 0, 0],
                  [mAfAs, mEuAs, 0, mChKHV, mChJp],
                  [0, 0, mChKHV, 0, 0],
                  [0, 0, mChJp, 0, 0]])
