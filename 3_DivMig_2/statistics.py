...
for i in os.listdir('results/dadi_pipeline_first/'):
	if not os.path.isfile('results/dadi_pipeline_first/' + str(i) + '/popt.npy'):
		continue
	with open('results/dadi_pipeline_first/' + str(i) + '/log_file.log') as f:
		for line in f:
	...
	with open('results/dadi_pipeline_first/' + str(i) + '/prefix.our_model.log.txt') as f:
                last_iter = 0
		for line in f:
...
dp_times = dp_times[:10]
dp_iters = dp_iters[:10]
