...
def Optimize_Routine(fs, pts, outfile, model_name, func, rounds, param_number, fs_folded=True, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    ...
    for r in range(rounds):
        ...
        for rep in range(1, (reps_list[r]+1) ):
            ...
            func_exec = func #dadi.Numerics.make_extrap_log_func(func)

            
            #perturb starting parameters
            params_perturbed = moments.Misc.perturb_params(best_params, fold=folds_list[r], upper_bound=upper_bound, lower_bound=lower_bound)

            #optimize from perturbed parameters
            params_opt = moments.Inference.optimize_powell(params_perturbed, fs, func_exec, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "{}.log.txt".format(model_name))
            print "\t\t\tOptimized parameters = ", params_opt

            #simulate the model with the optimized parameters
            sim_model = func_exec(params_opt, fs.sample_sizes)
    ...
    #cleanup file
    if os.path.isfile("{}.log.txt".format(model_name)):
